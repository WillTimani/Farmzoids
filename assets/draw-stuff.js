...
function draw_grid( rminor, rmajor, rstroke, rfill  ) 
{
    ...
    for ( var ix = 0; ix < width; ix += rminor )
    {
        ...
        line( ix, 0, ix, height );
        strokeWeight( 1 );
        if ( ix % rmajor == 0 ) { text( ix/20, ix, 10 ); } // text represents cell number (multiply by 20 to get real coordinate value)
    }
    for ( var iy = 0; iy < height; iy += rminor )
    {
        ...
        line( 0, iy, width, iy );
        strokeWeight( 1 );
        if ( iy % rmajor == 0 ) { text( iy/20, 0, iy + 10 ); } // text represents cell number (multiply by 20 to get real coordinate value)
    }
}
